...
class PerditaUpdate {
	public static function update<Model:PerditaModel, Msg>(appUpdate :Msg -> Model -> Bool, msg:PerditaMsg<Msg>, model:Model):Bool {
		return switch msg {
			case APP(appmsg): appUpdate(appmsg, model);
			case UI(uimsg): {
				switch uimsg {
	...
	public static inline function stretchColumn(model:PerditaModel, x :Int, y :Int) : Bool
	{
	...
	public static inline function checkForColumn(model:PerditaModel) : Bool
	{
		if(model.stretchableColumn != null) {
